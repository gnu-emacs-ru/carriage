...
(defun carriage-op-aibo-prompt-fragment (_ctx)
  ...
  (concat
   ...
   ";; patch history: RELATIVE/PATH — Короткое описание\n"
   "#+begin_patch (:version \"1\" :op \"aibo\" :file \"RELATIVE/PATH\")\n"
   "#+pair (:occur all :expect K) ; optional, applies to the NEXT pair\n"
   "#+begin_from\nFROM text\n#+end_from\n"
   "#+begin_to\nTO text\n#+end_to\n"
   "#+end_patch\n"
   "- STRICT FORMAT: write block markers exactly as shown, one marker per line.\n"
   "- The opening header must stay on the SAME line: '#+begin_patch (:version \"1\" :op \"aibo\" :file \"...\")'.\n"
   "- Do NOT move the header parentheses '(:version ...)' to the next line.\n"
   "- Do NOT split '#+begin_patch', '#+pair', '#+begin_from', '#+end_from', '#+begin_to', '#+end_to', or '#+end_patch'.\n"
   "- Each begin/end marker must be a complete single line with no detached brackets on following lines.\n"
   "- CORRECT example header: '#+begin_patch (:version \"1\" :op \"aibo\" :file \"foo.txt\")'.\n"
   "- INCORRECT: '#+begin_patch' on one line and '(:version \"1\" :op \"aibo\" :file \"foo.txt\")' on the next line.\n"
