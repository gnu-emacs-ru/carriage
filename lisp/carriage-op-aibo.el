...
(defun carriage-op-aibo-prompt-fragment (_ctx)
  ...
  (concat
   ...
   "\n"
   "CRITICAL RULES:\n"
   "- NO regex; :match is FORBIDDEN (literal-only).\n"
   "- For :occur all, :expect is REQUIRED (integer >= 0).\n"
   "- Empty TO block is ALLOWED (delete matched text).\n"
   "- NO unified diff; NO :op \"patch\" blocks.\n"
   "- NO prose outside the #+begin_patch block.\n"
   "- NO heading before the patch (do NOT start with '*' or '**').\n"
   "- Do NOT copy the `;; patch history:` format from input context — that is HISTORY, not OUTPUT.\n"
   "- Do NOT generate no-op patches (FROM == TO).\n"
   "- If no changes needed, output NOTHING (no patch block).\n"
   "\n"
   "EXAMPLE (correct):\n"
   ";; patch history: lisp/example.el — Fix typo\n"
