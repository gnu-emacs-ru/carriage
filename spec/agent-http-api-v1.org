...
- Enforce request-size limit for JSON bodies (e.g., POST /api/cmd) and reject oversized bodies as WEB_E_PAYLOAD.
...
- Enforce SSE client caps and idle-prune; on cap overflow, reject handshake with a structured error (code WEB_E_CAP or WEB_E_LIMITS).
- Secrets (tokens, raw prompts) MUST NOT be logged; logs are redacted/minimal.
