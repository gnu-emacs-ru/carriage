...
- REQ-style-009: Elisp files (.el) MUST NOT exceed 800 lines of code (FREEZE). If logic requires more, it MUST be split into semantic modules named as carriage-<domain>.el.
...
- Clean core + thin ports:
  ...
  - Modules MUST be minimally sufficient in size; each file MUST encapsulate a single responsibility.
- Async-first:
...
- INV-style-003: Elisp filenames MUST reflect their primary domain function and follow the pattern carriage-<domain>.el.
...
- ALG-style-004: file-size-check
  - Count non-comment, non-blank lines in .el files; fail if > 800.
...
- ERR-style-005: STYLE_E_FILE_SIZE — file exceeds the 800-line limit.
...
- CHK-style-004: Elisp files ≤ 800 lines of code; decomposition into semantic modules is enforced.
...
- Related: spec/index.org, spec/file-header-format-v2.org
