...
- Stability: stable
- Pack-Priority: P1
- Sources: doc (from #+begin_context plus optional patched-files from #+begin_patch headers), gptel (registry/files), visible (open buffers).
- State Manifest: a document block =#+begin_state_manifest … #+end_state_manifest= with rows =path|exists|has_text= describing whether a repo-relative file exists in the current project snapshot and whether its full current text is included in this request.
- State Manifest: a document block =#+begin_state_manifest … #+end_state_manifest= with rows =path|exists|has_text= describing whether a repo-relative file exists in the current project snapshot and whether its full current text is included in this request.
...
- REQ-ctx-009: Project Map source MUST be supported (default=on) that adds a `#+begin_map` block containing a gitignore-aware tree of repo-relative paths.
  - Preferred mechanism: `git ls-files -co --exclude-standard` (fast, gitignore-aware).
  - If git is unavailable, not a repo, or fails/timeouts, the implementation MAY fall back to OS/Emacs enumeration (directory traversal) to avoid hard failure.
    - When falling back, the implementation MUST warn that gitignore compliance may be incomplete.
  - The map MUST be deterministic (stable sort, stable rendering) and MUST NOT read file contents.
  - The map MUST be subject to limits (bytes/paths) and MAY be cached with TTL; overflows MUST be truncated deterministically and MUST warn.
  - If all methods fail or result is empty, the map MUST be omitted with a warning (no hard error).
- REQ-ctx-010: State Manifest source SHOULD be supported via a `#+begin_state_manifest` block with header row `path|exists|has_text` and deterministic repo-relative rows.
- REQ-ctx-011: `exists` and `has_text` MUST be treated as distinct facts; presence in `#+begin_map` or `#+begin_context` does not imply `has_text=true`.
- REQ-ctx-012: Edit-like responses MUST be considered valid only for files with `exists=true` and `has_text=true` in the current request state.
- REQ-ctx-013: If a required file has `exists=true` and `has_text=false`, the preferred safe fallback is to request that file via `#+begin_context` instead of generating edits from inference.
  - Preferred mechanism: `git ls-files -co --exclude-standard` (fast, gitignore-aware).
  - If git is unavailable, not a repo, or fails/timeouts, the implementation MAY fall back to OS/Emacs enumeration (directory traversal) to avoid hard failure.
    - When falling back, the implementation MUST warn that gitignore compliance may be incomplete.
  - The map MUST be deterministic (stable sort, stable rendering) and MUST NOT read file contents.
...
- INV-ctx-003: File existence and file-text availability are separate invariants; `exists=true` MUST NOT be interpreted as `has_text=true`.
...
- State Manifest block:
  - Marker lines: "#+begin_state_manifest" and "#+end_state_manifest"
  - First body line: `path|exists|has_text`
  - Following lines: `<repo-relative-path>|<true|false>|<true|false>`
...
- ALG-ctx-002: Format
  ...
  - Include `#+begin_state_manifest` when available before file bodies so downstream prompt builders and validators can distinguish `exists` from `has_text`
...
- ALG-ctx-004: Missing-text fallback
  - If a downstream action requires a file with `exists=true` and `has_text=false`, the system SHOULD prefer a deterministic `#+begin_context` request for the missing path(s) over speculative edits
