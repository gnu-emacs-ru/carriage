...
- Any discovery/limits-related constants MUST be defined only via Swarm FREEZE in spec/swarm-v1.org (single source of truth).
...
- Specs/docs MUST reference Swarm FREEZE *names* for limits/caps/timeouts and MUST NOT restate those numeric limit values anywhere outside spec/swarm-v1.org; implementations MAY repeat numeric limit values when required but MUST remain consistent with Swarm FREEZE and SHOULD centralize them in code.
