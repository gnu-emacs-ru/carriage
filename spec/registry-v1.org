...
- Specs/docs MUST reference Swarm FREEZE *names* and MUST NOT restate the numeric values anywhere outside spec/swarm-v1.org; implementations MAY repeat values when required but MUST remain consistent with Swarm FREEZE and SHOULD centralize them in code.
