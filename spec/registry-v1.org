...
- Version: v1.0
- Status: draft
- Readiness: alpha (layout and atomicity defined; needs hard guarantees for stale+GC behavior)
...
- Cleanup:
  - Stale detection MUST be bounded and deterministic (Hub-owned):
    ...
    - PID alive but health probe fails ⇒ mark alive=false only after bounded failure budget:
      - Probe uses bounded timeout (see Swarm FREEZE proxy-connect-timeout-sec/proxy-read-timeout-sec).
      - Probe attempts per tick MUST be capped (see Swarm FREEZE names; Hub spec defines scheduling).
