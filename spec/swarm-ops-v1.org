#+title: Swarm Operations v1 — Supervisor, runtime layout, lifecycle
...
- Version: v1.0
- Status: stable
- Gate: ГОТОВО (ready to implement; main Emacs is process-only; hub-owned liveness; no sync network calls)
- Readiness: beta (spec-pass complete; READY/ГОТОВО for implementation; tests must prove no sync network in Main Emacs and hub-owned liveness)
...
- INV-ops-005: Hub SHOULD own liveness probing and registry last_seen/alive updates; main Emacs MUST NOT run periodic probes.
  - Probing schedule and budgets MUST be bounded by Swarm FREEZE names: probe-interval-sec, probe-timeout-sec, probe-max-per-interval.
- INV-ops-006: Operational limits (caps/timeouts) MUST reference Swarm FREEZE names (spec/swarm-v1.org#INV), the single source of truth for numeric limits.
- INV-ops-007: Legacy webd/web-supervisor/push modules and operational paths MUST NOT exist in Swarm baseline; only Agent/Hub/Registry are allowed.
