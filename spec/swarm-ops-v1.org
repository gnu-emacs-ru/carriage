...
- INV-ops-006: Operational limits (caps/timeouts) MUST match Swarm FREEZE values in spec/swarm-v1.org; duplicate/conflicting limits in code/specs/docs are forbidden.
  - Specs and docs MUST reference the Swarm FREEZE *names*; duplicating numeric literals outside spec/swarm-v1.org is forbidden in specs/docs.
- Implementations SHOULD centralize these values in code; repeating literals is allowed when required by runtime constraints, but MUST remain consistent with Swarm FREEZE.
