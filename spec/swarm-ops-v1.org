...
- INV-ops-006: Operational limits (caps/timeouts) MUST match Swarm FREEZE values in spec/swarm-v1.org (single source of truth).
  - Specs/docs MUST reference the Swarm FREEZE *names* for limits/caps/timeouts and MUST NOT restate those numeric limit values anywhere outside spec/swarm-v1.org (no duplicates, no alternative constants).
- Implementations MAY repeat numeric values when required by runtime constraints, but SHOULD centralize them in code and MUST remain consistent with Swarm FREEZE.
