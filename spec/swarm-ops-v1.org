...
- INV-ops-006: Operational limits (caps/timeouts) MUST match Swarm FREEZE values in spec/swarm-v1.org (single source of truth).
  - Specs/docs MUST reference the Swarm FREEZE *names* and MUST NOT restate the numeric values anywhere outside spec/swarm-v1.org (no duplicates, no alternative constants).
