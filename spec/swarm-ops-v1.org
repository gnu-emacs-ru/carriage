...
- INV-ops-006: Operational limits (caps/timeouts) MUST reference Swarm FREEZE names (spec/swarm-v1.org#INV), which are the single source of truth.
  - Specs/docs MUST NOT restate the numeric values outside spec/swarm-v1.org#INV (exceptions: protocol vocabulary such as HTTP status codes, standard media types, and header names; plus file permission bits such as 0600).
