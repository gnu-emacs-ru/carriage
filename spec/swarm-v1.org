...
- Version: v1.0
- Status: draft
- Readiness: alpha (spec-pass in progress; implementation/tests must prove bounded proxy + liveness ownership)
...
- FREEZE values in this section are the single source of truth for Swarm limits/caps/timeouts.
  - Specs/docs MUST reference FREEZE *names* (not numbers) and MUST NOT restate the numeric values outside this FREEZE list.
    - Exceptions: protocol vocabulary such as HTTP status codes, standard media types, and header names; plus file permission bits such as 0600.
