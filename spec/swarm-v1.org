...
- Version: v1.0
- Status: stable
- Gate: ГОТОВО (specs are internally consistent; ready to implement; tests must confirm bounded proxy + hub-owned liveness)
- Readiness: beta (spec-pass complete; READY/ГОТОВО for implementation; tests must prove bounded proxy + liveness ownership)
- Dialectic-Readiness (summary):
  - Thesis: Separation of concerns is specified (Agent serves; Hub proxies/injects token; Registry discovers; Main Emacs orchestrates only).
  - Antithesis (risk): Any ambiguity in (a) liveness ownership, (b) SSE proxy boundedness, (c) token non-leakage reintroduces coupling and UI stalls.
  - Synthesis (gate): Ready when FREEZE is the only numeric source of truth, Hub owns probes+registry updates, proxy is bounded (caps/timeouts/no buffering), and auth/envelopes are uniform.
- Ready-For: Spec-level conformance tests (Agent auth, Hub token-injection, Registry atomicity, Proxy boundedness)
- Not-Ready-For: Broad UI/keys exposure beyond start/stop/open-dashboard (until Hub proxy + liveness model is proven by tests)
...
- FREEZE values in this section are the single source of truth for Swarm limits/caps/timeouts.
  - Specs/docs MUST reference FREEZE names (not numbers) and MUST NOT restate numeric limit values outside this FREEZE list.
    - Exceptions: protocol vocabulary (HTTP status codes, header names, standard media types) and file permission bits (e.g., 0600) may be stated where required.
...
- FREEZE: probe-interval-sec = 5.0
- FREEZE: probe-timeout-sec = 0.5
- FREEZE: probe-max-per-interval = 12
...
- Anchor: TST
- Dialectic-Readiness:
  - Thesis: Roles are separated (Agent serves; Hub proxies; Registry discovers; Main Emacs orchestrates only).
  - Antithesis: Any ambiguity in liveness ownership, proxy boundedness (esp. SSE), or token handling reintroduces coupling and UI stalls.
  - Synthesis: Conformance is achieved when FREEZE is the only numeric source of truth, Hub owns liveness updates, proxy is fully bounded, and tokens never reach browser.
- Ready-For:
  - Integration tests that start Agent + Hub and assert: auth-on-all-agent-endpoints, hub token-injection, bounded proxy caps/timeouts, atomic registry rewrite, stale cleanup.
- Not-Ready-For:
  - Expanding command surface beyond strict whitelist; any “arbitrary eval/RPC”.
...
- Compliance Checklist (minimum):
  ...
  - CHK-swarm-008: SSE reconnects tolerated; clients capped and idle-pruned; Hub forwards SSE without unbounded buffering and disconnects slow clients (bounded by Swarm FREEZE proxy-read-timeout-sec).
- CHK-swarm-009: Legacy webd/web-supervisor/push paths are forbidden; Swarm uses only Agent/Hub/Registry roles and their specified interfaces.
