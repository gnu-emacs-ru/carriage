...
- Stability: stable
- Pack-Priority: P1
- FREEZE values in this section are the single source of truth for Swarm.
  - Specs/docs MUST reference these FREEZE *names* and MUST NOT restate the numeric values anywhere outside this section (no duplicates, no alternative constants).
  - Implementations MAY repeat the numeric values when required by runtime constraints, but MUST remain consistent with the Swarm FREEZE values and SHOULD centralize them in code.
