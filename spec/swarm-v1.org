...
- Stability: stable
- Pack-Priority: P1
- FREEZE values in this section are the single source of truth for Swarm (single source of truth).
  - Specs/docs MUST reference these FREEZE *names* for all limits/caps/timeouts and MUST NOT restate the numeric values outside this section.
    - Exception: protocol vocabulary (HTTP status codes, header names, and standard media types) may be stated where required.
  - Implementations MAY repeat numeric values when required by runtime constraints, but MUST remain consistent with the Swarm FREEZE values and SHOULD centralize them in code.
  - Specs/docs MUST reference these FREEZE *names* for all limits/caps/timeouts and MUST NOT restate those numeric limit values anywhere outside this section.
    - Exception: protocol vocabulary (HTTP status codes, header names, and standard media types) may be stated where required.
  - Implementations MAY repeat numeric limit values when required by runtime constraints, but MUST remain consistent with the Swarm FREEZE values and SHOULD centralize them in code.
