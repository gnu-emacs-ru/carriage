...
- Ask/Hybrid system prompts SHOULD instruct the model to wrap the key parts of the reply into Typed Blocks (v1):
  begin_task, begin_analysis, begin_plan, begin_verify, begin_commands, begin_question, begin_answer, begin_context
  (begin_notes is optional).
- Only important information SHOULD go inside blocks so the tool can extract it even when plain text is disabled.
- Keep prose minimal and avoid duplicating the same content both inside and outside the blocks.
