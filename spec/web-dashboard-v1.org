...
- HTTP endpoints (JSON; Content-Type: application/json; UTF-8):
  ...
  - GET /api/metrics
    - 200: {"ok":true,"data":{
            "published": int,          ; SSE deliveries (best-effort)
            "truncated": int,          ; JSON truncations due to size limits
            "clients": int,            ; current SSE clients
            "sessions_count": int,     ; entries in sessions cache
            "sessions_age_ms": number|null, ; age of sessions cache in ms (null when unknown)
            "ts": number               ; server timestamp (unix, float)
          }}
  - GET /stream[?doc=<id>]
    - SSE events:
...
- Code:
  ...
  - lisp/carriage-web-supervisor.el (supervisor for external Emacs web daemon)
